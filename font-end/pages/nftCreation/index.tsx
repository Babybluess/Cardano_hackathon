...
import { pinFiles, shortUrlImage } from "@/utils/pinningData/pinata";
import { assetService, auctionService, userService } from "@/service";
import { auctionCode } from "../const";
import { createAsset } from "@/role/AuctionStage/createAsset";
...
function index() {
   ...
   const [url, setUrlNFTLocation] = useState("");
   ...
   const createNFT = async () => {
      const user = await userService.getProfileByName(address)

      if (user.market !== null) {
         const pinnedFile = await pinFiles(url);
         const shortedUrl = await shortUrlImage(pinnedFile);
         ...
         const nft = await wallet.getAssets();
         const nftId = nft[nft.length - 1].unit;

         const newAsset = await createAsset(nftId, startAt, endAt, address, pinnedFile, typeNFT);
            ...
            <section className=" flex flex-col gap-10">
               ...
               <div className=" flex flex-col gap-10">
                  ...
                  <div className=" flex flex-col gap-2">
                     ...
                     <p className="text-gray-600 text-sm">
                        Enter the listed time of auction
                     </p>
                     <input
                        required
                     ...
                     <div className=" w-full flex gap-2">
                        <div className=" w-6 h-6">
                           <img
                              ...
                              src={"./images/bannerIMG/avatar.jpg"}
                           />
                        </div>
                        <p>
                           {address.substring(0, 6)}...{address.substring(100)}
                        </p>
                     </div>
